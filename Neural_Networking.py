...
class NeuralNet:
    def __init__(self):
        ...
        self.neuronnum = [4,2,1] #neurons for each layer
        self.inputnum = [1,4,2] #inputs for each layer
    ...
    def Forward(self, Inputs):
        ...
        for i in range(0,len(Inputs)): #cycles through the inputs so each neuron in the input layer has its own input
            Processing[0].append(self.Layers[0][i].ActivationFunction(Inputs[i])) #runs inputs thtrough activation function and adds outputs to processing
